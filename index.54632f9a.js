...
})({"6zsCq":[function(require,module,exports) {
...
var _moduleEsm = require("../node_modules/alpinejs/dist/module.esm");
var _moduleEsmDefault = parcelHelpers.interopDefault(_moduleEsm);
(0, _moduleEsmDefault.default).store("control", {
    currentDeviceName: "",
    password: "",
    controles: [],
    edit: false,
    ...
    getDevices() {
        navigator.bluetooth
            ...
            .then((device) => {
                const deviceName: string = device.name as string;
                localStorage.setItem(deviceName, "opengate");
                this.updateList(deviceName);
    ...
    */ init () {
    ...
    saveControl () {
        if (this.currentDeviceName != "" && this.password != "") {
            //console.log("stuff" + this.currentDeviceName + this.password);
            localStorage.setItem(this.currentDeviceName, this.password);
            this.updateList(this.currentDeviceName);
        }
        this.password = "";
        this.currentDeviceName = "";
    },
    updateList (name) {
        if (!this.controles.includes(name)) this.controles.push(name);
    },
    borrar (name) {
        localStorage.removeItem(name);
        const index = this.controles.indexOf(name, 0);
        if (index > -1) this.controles.splice(index, 1);
        this.edit = false;
    },
    async connectToDevice (deviceName) {
        try {
            ...
            const pass = localStorage.getItem(deviceName);
            var response = await characteristic.writeValue(enc.encode(pass));
...
(0, _moduleEsmDefault.default).start();

},{"../node_modules/alpinejs/dist/module.esm":"69hXP","@parcel/transformer-js/src/esmodule-helpers.js":"gkKU3"}],"69hXP":[function(require,module,exports) {
...
},{}]},["6zsCq","gg0zR"], "gg0zR", "parcelRequiree90c")
